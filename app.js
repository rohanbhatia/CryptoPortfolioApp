...
app.post('/transaction', function (req, res) {
	...
	fs.readFile(__dirname + '/tx_auth.txt', 'utf8', function (err,data) {
  		...
  		if (password.toString().trim() == data.toString().trim()) { //authenticated
  			...
  			var query = "INSERT INTO buys (date, cost, crypto_type, crypto_amount, effective_price, actual_price) VALUES ( STR_TO_DATE('" + transaction_date + "', '%Y-%m-%d'), " + transaction_cost + ", '" + crypto_type + "', " + crypto_amount + ", " + effective_price.toString() + ", " + current_price + ")";
